@php($title = 'Products')
@php($subtitle = 'Add product')
@extends('layouts.admin')

@section('content')
  <div class="flex items-start justify-between gap-3">
    <div>
      <h1 class="text-xl font-semibold tracking-tight">Add product</h1>
      <p class="mt-1 text-sm text-zinc-600">Sizes: comma-separated (e.g. 39, 40, 41). Leave blank for default shoe sizes.</p>
    </div>
    <a href="{{ route('admin.products.index') }}" class="text-sm font-semibold text-zinc-700 hover:text-zinc-950">Back</a>
  </div>

  <form method="post" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="mt-4 grid gap-3 lg:grid-cols-3">
    @csrf
    <section class="lg:col-span-2 rounded-2xl border border-zinc-200 bg-white p-4 space-y-3">
      <div>
        ...
        <input id="name" name="name" value="{{ old('name') }}" required class="mt-2 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-zinc-400" />
      </div>
      <div>
        <label class="text-sm font-semibold" for="slug">Slug (optional)</label>
        <input id="slug" name="slug" value="{{ old('slug') }}" class="mt-2 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-zinc-400" />
      </div>
      <div>
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <input id="price_mmk" name="price_mmk" type="number" min="0" step="1" value="{{ old('price_mmk') }}" required class="mt-2 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-zinc-400" />
        </div>
        <div>
          <label class="text-sm font-semibold" for="badge">Badge</label>
          <input id="badge" name="badge" value="{{ old('badge') }}" class="mt-2 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-zinc-400" placeholder="New, Offer…" />
        </div>
      </div>
      <div class="grid gap-3 sm:grid-cols-2">
        <div>
        ...
        <div class="flex items-end pb-1">
          <label class="flex items-center gap-2 text-sm font-semibold text-zinc-700">
            ...
            <input type="checkbox" name="is_active" value="1" class="rounded border-zinc-300" @checked(old('is_active', true)) />
      ...
      <div>
        ...
        <input id="size_options" name="size_options" value="{{ old('size_options') }}" class="mt-2 h-11 w-full rounded-xl border border-zinc-200 bg-white px-3 text-sm outline-none focus:border-zinc-400" placeholder="39, 40, 41, 42" />
      </div>
      <div>
