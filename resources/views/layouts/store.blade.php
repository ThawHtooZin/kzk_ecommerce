      ...
      <header class="sticky top-0 z-30 bg-slate-900/95 text-white backdrop-blur border-b border-white/10">
        <div class="px-4 py-3">
          <div class="flex items-center gap-3">
            <button type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 lg:hidden" data-mobile-menu-trigger>
            ...
            <a href="{{ url('/') }}" class="flex min-w-0 items-center gap-2">
              ...
              <div class="leading-tight min-w-0">
                ...
                <div class="text-[11px] text-white/70 -mt-0.5 truncate" data-i18n="nav.tagline">Myanmar · Prices in MMK</div>
              </div>
            </a>

            <div class="flex-1"></div>

            <div class="flex items-center gap-0.5 rounded-xl border border-white/10 p-0.5 text-[11px] font-bold leading-none sm:text-xs">
              <button type="button" data-locale-switch="en" class="rounded-lg px-2 py-1.5 text-amber-200 hover:bg-white/10" data-i18n="nav.lang_en">EN</button>
              <button type="button" data-locale-switch="my" class="rounded-lg px-2 py-1.5 text-amber-200 hover:bg-white/10" data-i18n="nav.lang_my">မြန်မာ</button>
            </div>
            ...
            <a href="{{ url('/cart') }}" data-store-cart-wrap class="relative inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 text-sm font-medium text-white">
              <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <div class="lg:hidden" data-mobile-menu hidden>
          <div class="border-t border-white/10 px-4 py-3 grid grid-cols-2 gap-2 text-sm">
            ...
            <a class="rounded-xl px-3 py-2 hover:bg-white/10 {{ request()->is('contact') ? 'bg-white/10 font-semibold' : '' }}" href="{{ url('/contact') }}" data-i18n="nav.contact">Contact</a>
            @guest
              <a class="rounded-xl px-3 py-2 hover:bg-white/10" href="{{ url('/login') }}" data-i18n="nav.sign_in">Sign in</a>
              <a class="rounded-xl px-3 py-2 hover:bg-white/10 font-semibold text-amber-200" href="{{ url('/register') }}" data-i18n="nav.register">Register</a>
            @endguest
            <a class="col-span-2 rounded-xl px-3 py-2 hover:bg-white/10" href="{{ url('/admin') }}" data-i18n="nav.admin">Admin</a>
          </div>
        </div>
      </header>
